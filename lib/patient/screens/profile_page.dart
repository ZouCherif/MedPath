...
class _ProfilPage1State extends State<ProfilPage1> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      ...
      return FutureBuilder(
        ...
        return SafeArea(
          child: Scaffold(
              floatingActionButton:  snapshot.data == "docteur"
  ...
  void _pickFile(id) async {
    // opens storage to pick files a
    final result = await FilePicker.platform.pickFiles(allowMultiple: false);

    // if no file is picked
    if (result == null) return;
    // generate file name with random number
    final fileName = Random().nextInt(1000).toString();
    // save file to firebase storage
    Reference docRef = FirebaseStorage.instance.ref("$id/document").child("$fileName");
